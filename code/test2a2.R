...
outfile <- paste0(plotpath,program,progver,"_",
tolower(str_replace_all(params[["title"]]," ","_")))
# Plot data.
mseplot <- 
ggplot(dt_res,aes_string(params[["xvar"]],params[["yvar"]]))+
geom_line(color=params[["line_color"]],size=params[["line_size"]])+
theme_light(base_size=params[["base_font_size"]])+
ylab(params[["ylab"]])+
xlab(params[["xlab"]])+
labs(title=params[["title"]],
  caption=paste0(
    ...
    "Generated by: ",paste0(program,progver)
  )
)+
theme(plot.caption=element_text(hjust=0,size=params[["caption_font_size"]]),
  axis.text=element_text(colour="black",size=params[["base_font_size"]]),
  plot.title=element_text(size=params[["title_font_size"]]))+
scale_y_continuous(limits=c(params[["ylim_min"]],params[["ylim_max"]]))
# Export plot.
ggsave(paste0(outfile,".png"),plot=mseplot,width=params[["width"]],
height=params[["height"]])
ggsave(paste0(outfile,".pdf"),plot=mseplot,width=params[["width"]],
height=params[["height"]])
# Show status.
cat("\n***** Plot exported:",outfile,"\n")
